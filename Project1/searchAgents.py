...
def cornersHeuristic(state, problem):
    ...
    for r in range(len(unvisited)):
        dist=[]
        for corner in unvisited:
            dist.append(util.manhattanDistance(current, corner))
            if util.manhattanDistance(current, corner) == min(dist):
...
def foodHeuristic(state, problem):
    ...
    for food in foodList:
        ...
        if util.manhattanDistance(position, food) == min(dist):
            nearestfood = food
    return mazeDistance(position, nearestfood, problem.returngamestate())
